...
static int pp_read_operations(struct policy_file *fp, policy_avtab_operations_t *avtab_op)
{
	...
	rc = next_entry(&(avtab_op->specified), fp, sizeof(avtab_op->specified));
	if (rc) {
		return rc;
	}

	rc = next_entry(&(avtab_op->driver), fp, sizeof(avtab_op->driver));
	if (rc) {
		return rc;
	}
...
static int pp_write_operations(policy_avtab_operations_t *avtab_op, struct policy_file *fp)
{
	(void)put_entry(&(avtab_op->specified), sizeof(avtab_op->specified), 1, fp);
	(void)put_entry(&(avtab_op->driver), sizeof(avtab_op->driver), 1, fp);
