...
//extern int tp_gpio_reset;//add by wkh
static int BLMAX_LCM=0x0;               //modify by yujianhua start for V50 lcm bl level 0xfff
static int BLMAX_LCM_10BIT=0;
static int LCM_RST_HIGH=0x1;		//modify by yujianhua start for f10 lcm rst high in sleep mode
static bool lcm_rst_binding_tp_lpwg=false;
...
{		
	...
	if (!strcmp(panel_name, "zfgLEAD(hx8399c+auo)_1080*1920-5.2Inch"))
	{
		BLMAX_LCM=0x1;
		LCM_RST_HIGH=0x1;
	}
	...
	if (!strcmp(panel_name, "zfg(otm1911a)_1080*1920-5.2Inch"))
	{
		BLMAX_LCM_10BIT=1;
		LCM_RST_HIGH=1;
	}
	
	if (!strcmp(panel_name, "zfgBOE(td4300)_1080*1920-5.5Inch"))
	{
		LCM_RST_HIGH=0x1;
		lcm_rst_binding_tp_lpwg=true;
	}

	if (!strcmp(panel_name, "zfgTianMa(td4310)_1080*1920-5.5Inch"))
	{
		LCM_RST_HIGH=0x1;
		lcm_rst_binding_tp_lpwg=true;
	}
	
	if (!strcmp(panel_name, "zfgSkyworth(ili9881c)_720*1280-5.0Inch"))
	{
		LCM_RST_HIGH=0x0;
	}
}
...
static char led_dcs1[3] = {0x51,0x00,0x00};	/* DTYPE_DCS_WRITE1 */
...
static u8 LCM_MINBRIGHT=0;
static u8 LCM_CABC=1;
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	//add end
		...
		pr_err("%s:MAXLCM level=%d,cabc=%d,minb=%d\n", __func__, level,LCM_CABC,LCM_MINBRIGHT);
	}
	else if(BLMAX_LCM_10BIT==1)
	{
		if ( (level <= 24) && (level > 0))    
		{
			level = 16;
		...
		led_dcs1[1] = (unsigned char)(level>>2)&0xff;
		led_dcs1[2] = (unsigned char)(level<<2)&0x0f;
		pr_err("%s:MAXLCM level=%d,cabc=%d,minb=%d\n", __func__, level,LCM_CABC,LCM_MINBRIGHT);
	}
	else
	{
	...
	led_pwm1[1] = (unsigned char)level;
              pr_err("%s: level=%d,cabc=%d,minb=%d\n", __func__, level,LCM_CABC,LCM_MINBRIGHT);		
	}

	memset(&cmdreq, 0, sizeof(cmdreq));

	if(BLMAX_LCM==0x1 || BLMAX_LCM_10BIT==0x1)
	{
...
//add by yujianhua for dcs backlight ctrl minimum brightness start
	...
	else if((LCM_MINBRIGHT==0)&&(LCM_CABC==0))
	{
		led_cabc1[1] = 0x1;
		cmdreq.cmds = &backlight_cmd_cabc;
		cmdreq.cmds_cnt = 1;
...
//add by yujianhua for dcs backlight ctrl brightness brightness end
}
...
extern bool tp_enable_wakeup_gesture_state;
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if(!lcm_rst_binding_tp_lpwg)
		{
			if(LCM_RST_HIGH==0x1)
			{
				pr_err("%s: LCM Reset high\n", __func__);
				gpio_set_value((ctrl_pdata->rst_gpio), 1);
			}
			else
			{
				pr_err("%s: LCM Reset low\n", __func__);
		...
		else
		{
			...
			{
				pr_err("%s: TP enter WG state, LCM Reset high\n", __func__);
				gpio_set_value((ctrl_pdata->rst_gpio), 1);
			}
			else
			{
				pr_err("%s: TP exit WG state, LCM Reset low\n", __func__);
				gpio_set_value((ctrl_pdata->rst_gpio), 0);
			}
		}
		//modify by yujianhua start for F10 lcm rst high in sleep mode
