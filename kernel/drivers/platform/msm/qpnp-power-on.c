...
int zfg_warm_reset = 0;
static int __init zfg_warm_reset_setup(char *str) 
{
	if(get_option(&str, &zfg_warm_reset))
	{
	    printk("zfg_warm_reset = %d\n", zfg_warm_reset);
	    return 0;
	}
	else
	{
	    printk("zfg_warm_reset get failed \n");
	    return -EINVAL;
	}
}
early_param("zfg_warm_reset", zfg_warm_reset_setup);
...
int qpnp_pon_is_warm_reset(void)
{
    ...
    dev_info(&pon->spmi->dev, "PMIC@SID%d Warm-reset reason1: 0x%x\n", pon->spmi->sid, pon->warm_reset_reason1);
    dev_info(&pon->spmi->dev, "PMIC@SID%d Warm-reset reason2: 0x%x\n", pon->spmi->sid, pon->warm_reset_reason2);
...
/* Modified by ShiXingchuan, set PON_POWER_OFF_WARM_RESET for PM_RESIN */
...
static irqreturn_t qpnp_resin_bark_irq(int irq, void *_pon)
{
    ...
    /* Modified by ShiXingchuan, comment original implementation */
	//input_report_key(pon->pon_input, cfg->key_code, 1);
	//input_sync(pon->pon_input);
    /* End Modified by ShiXingchuan */
...
static int qpnp_pon_config_init(struct qpnp_pon *pon)
{
	...
	for_each_available_child_of_node(pon->spmi->dev.of_node, pp) {
		...
		if (cfg->support_reset) {
            ...
            if ((PON_KPDPWR == cfg->pon_type) && zfg_warm_reset)
            {
                cfg->s2_type = 1;
            }
...
/* Added by ShiXingchuan, set PON_POWER_OFF_WARM_RESET if debug is enabled */
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
    ...
    /* Added by ShiXingchuan, set PON_POWER_OFF_HARD_RESET as default */
