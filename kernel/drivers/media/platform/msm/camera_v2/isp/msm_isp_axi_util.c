	...
	struct msm_vfe_axi_halt_cmd *halt_cmd)
{
	...
	if (halt_cmd->stop_camif) {
        ...
        for (i = 0; i < VFE_SRC_MAX; i++) {
           vfe_dev->axi_data.src_info[i].running = 0;
        }
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (!wait_for_complete_for_this_stream ||
			...
			if (intf >= VFE_RAW_0 &&
				...
				vfe_dev->axi_data.src_info[intf].running = 0;
			}
		} else
			src_mask |= (1 << intf);
...
int msm_isp_restart_wms(struct vfe_device *vfe_dev,
                        struct msm_vfe_axi_stream_cfg_cmd *stream_cfg_cmd)
{
	int i = 0 , j = 0, k = 0, need_enable_wm = 1, rc = 0;
	struct msm_vfe_axi_shared_data *axi_data;
	struct msm_vfe_axi_stream *stream_info;
	struct dual_vfe_resource *dual_vfe_res = NULL;
	struct vfe_device *vfe_dev_cur = NULL;
	unsigned long flags;
	uint8_t wait_for_update;

	if (vfe_dev->is_split) {
                mutex_lock(&vfe_dev->common_data->common_mutex);
                if (!vfe_dev->axi_data.need_wm_reconfig) {
				   mutex_unlock(&vfe_dev->common_data->common_mutex);
                   return rc;
                }
		/*Dual ISP Use case. Restart all WMs in both ISPs*/
		dual_vfe_res = vfe_dev->common_data->dual_vfe_res;
		if (!dual_vfe_res) {
			pr_err("%s: Dual vfe resource not found\n", __func__);
                        mutex_unlock(&vfe_dev->common_data->common_mutex);
			return -EINVAL;
		}
		/*First stop All Write Masters in Both ISPs to avoid
			Ping pong issues*/
		for (i = 0; i < MAX_VFE; i++) {
			vfe_dev_cur = dual_vfe_res->vfe_dev[i];
			if (!vfe_dev_cur) {
				pr_err("%s: Vfe device not found\n", __func__);
                                mutex_unlock(&vfe_dev->common_data->common_mutex);
				return -EINVAL;
			}

			axi_data = &vfe_dev_cur->axi_data;
			for (j = 0; j < axi_data->hw_info->num_wm; j++) {
				if (axi_data->free_wm[j]) {
					vfe_dev_cur->hw_info->vfe_ops.axi_ops.
					enable_wm(vfe_dev_cur->vfe_base, j, 0);
				}
			}

		}

		/*After Stopping Wms, issue reg updates for both ISPs*/
		...
		for (i = 0; i < MAX_VFE; i++) {
			...
			axi_data = &vfe_dev_cur->axi_data;
			/*Issue Reg update forcefully*/
			vfe_dev_cur->hw_info->vfe_ops.core_ops.
			reg_update(vfe_dev_cur, VFE_SRC_MAX);

			/*Wait till the Next reg update */
			wait_for_update = 0;
			spin_lock_irqsave(&vfe_dev_cur->shared_data_lock,
					flags);
			init_completion(&vfe_dev_cur->stream_config_complete);
			vfe_dev_cur->axi_data.pipeline_update = NO_UPDATE;
			for (j = 0; j < VFE_AXI_SRC_MAX; j++) {
				stream_info =
					&vfe_dev_cur->axi_data.stream_info[j];
				if (stream_info->state == ACTIVE &&
					(vfe_dev_cur->axi_data.
					src_info[
					SRC_TO_INTF(stream_info->stream_src)].
					running)) {
					wait_for_update = 1;
			...
			spin_unlock_irqrestore(&vfe_dev_cur->shared_data_lock,
					flags);
			if (wait_for_update) {
				rc = wait_for_completion_timeout(
					&vfe_dev_cur->stream_config_complete,
					msecs_to_jiffies(VFE_MAX_CFG_TIMEOUT));
				if (rc == 0) {
					pr_err("%s: wait timeout\n", __func__);
					rc = -1;
			...
			/* Reconfigire UB as per the hw streams */
			vfe_dev_cur->hw_info->vfe_ops.axi_ops.cfg_ub(
				vfe_dev_cur);
		}
		...
		for (i = 0; i < MAX_VFE; i++) {
			...
			for (j = 0; j < axi_data->hw_info->num_wm; j++) {
				need_enable_wm = 1;
				if (vfe_dev->pdev->id == vfe_dev_cur->pdev->id) {
				   for (k = 0; k < stream_cfg_cmd->num_streams; k++) {
					   if (HANDLE_TO_IDX(
						   stream_cfg_cmd->stream_handle[k])
						   > MAX_NUM_STREAM) {
                                                   mutex_unlock(&vfe_dev->common_data->common_mutex);
						   return -EINVAL;
					   }
				...
				if (need_enable_wm && axi_data->free_wm[j]) {
					vfe_dev_cur->hw_info->vfe_ops.axi_ops.
						enable_wm(vfe_dev_cur->vfe_base, j, 1);
				}
			}
                        vfe_dev_cur->axi_data.need_wm_reconfig = 0;
		}
                mutex_unlock(&vfe_dev->common_data->common_mutex);

	} else {
		/* The UB needs to reconfigured for all the active wm
		as we are going to add one more hw stream*/
		/* Fisrt step is to stop all the WM which are active */
		axi_data = &vfe_dev->axi_data;
		for (i = 0; i < axi_data->hw_info->num_wm; i++) {
			if (axi_data->free_wm[i]) {
				vfe_dev->hw_info->vfe_ops.axi_ops.
					enable_wm(vfe_dev->vfe_base, i, 0);
			}
		}
                /*Issue Reg update forcefully*/
                vfe_dev->hw_info->vfe_ops.core_ops.reg_update(vfe_dev,
		...
		/* Start the wm of the previous active stream as the
			wm for the current stream will be started in stream on*/
		for (i = 0; i < axi_data->hw_info->num_wm; i++) {
			need_enable_wm = 1;
			for (j = 0; j < stream_cfg_cmd->num_streams; j++) {
				if (HANDLE_TO_IDX(
					stream_cfg_cmd->stream_handle[j])
					> MAX_NUM_STREAM) {
					return -EINVAL;
				}
				stream_info = &axi_data->stream_info[
			...
			if (need_enable_wm && axi_data->free_wm[i]) {
				vfe_dev->hw_info->vfe_ops.axi_ops.
					 enable_wm(vfe_dev->vfe_base, i, 1);
			}
...
int msm_isp_cfg_axi_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	if (axi_data->num_active_stream == 0) {
		...
		atomic_set(&vfe_dev->error_info.overflow_state,
			NO_OVERFLOW);
	} else if (axi_data->need_wm_reconfig) {
		msm_isp_restart_wms(vfe_dev, stream_cfg_cmd);
	}
