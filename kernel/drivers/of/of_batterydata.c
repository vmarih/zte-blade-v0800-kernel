		...
		const char *psy_name,  const char  *batt_type)
{
	...
	int delta = 0, best_delta = 0, best_id_kohm = 0, id_range_pct,
		batt_id_kohm = 0, batt_id_parallel_330k_kohm = 0, i = 0, rc = 0, limit = 0;
	bool in_range = false;
	...
	batt_id_parallel_330k_kohm = ret.intval / 1000;

// set id_kohm to 20, only for PV version. added by lixinghe 20161209. 
	batt_id_kohm = 330*batt_id_parallel_330k_kohm/(330-batt_id_parallel_330k_kohm);

	pr_err("batt_id_parallel_330k_kohm = %d, batt_id_kohm = %d\n",batt_id_parallel_330k_kohm, batt_id_kohm);
#if ZFG_FASTMMI_PV
	batt_id_kohm = 20;
#endif
	...
	for_each_child_of_node(batterydata_container_node, node) {
		if (batt_type != NULL) {
			...
			if (!rc && strcmp(battery_type, batt_type) == 0) {
				...
				pr_err("I found the batterydata, node:%s, best_id_kohm=%d\n",batt_type,batt_id_kohm);
				break;
			}
		} else {
