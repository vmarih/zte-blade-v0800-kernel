		...
		enum led_brightness value)
{
	...
	if (!send_info) {
		...
		snprintf(buffer, sizeof(buffer), "%d,%d", STATUS_LED->red_led - STATUS_LED->gpio_base, STATUS_LED->green_led - STATUS_LED->gpio_base);
		rc |= sysmon_send_led_cmd(SSCTL_SSR_EVENT_LEDS_SET_GPIO, buffer);

		snprintf(buffer, sizeof(buffer), "%d,%d,%d", STATUS_LED->blink_led[0].blink_flag, STATUS_LED->blink_led[0].blink_on_time, STATUS_LED->blink_led[0].blink_off_time);
		rc |= sysmon_send_led_cmd(SSCTL_SSR_EVENT_LEDS_SET_RED_BLINK, buffer);

		snprintf(buffer, sizeof(buffer), "%d,%d,%d", STATUS_LED->blink_led[1].blink_flag, STATUS_LED->blink_led[1].blink_on_time, STATUS_LED->blink_led[1].blink_off_time);
	...
	else if(gpio_is_valid(STATUS_LED->trickle_red_led)) //trickle
	{	
		if(value == LED_OFF)
                {
		...
		const char *buf, size_t size)
{
	...
	if (count == size) {
		...
		if(0 == state)
		{
			...
			snprintf(buffer, sizeof(buffer), "%d,%d,%d", STATUS_LED->blink_led[idx].blink_flag, STATUS_LED->blink_led[idx].blink_on_time, STATUS_LED->blink_led[idx].blink_off_time);
			if (idx == 0)
		...
		const char *buf, size_t size)
{
	...
	if (count == size) {
		ret = count;
		mutex_lock(&STATUS_LED->data_lock);
		STATUS_LED->blink_led[idx].blink_on_time = state;
		mutex_unlock(&STATUS_LED->data_lock);
	}
		...
		const char *buf, size_t size)
{
	...
	if (count == size) {
		ret = count;
		mutex_lock(&STATUS_LED->data_lock);
		STATUS_LED->blink_led[idx].blink_off_time = state;
		mutex_unlock(&STATUS_LED->data_lock);
	}
...
static int gpio_led_probe(struct platform_device *pdev)
{
	...
	printk("==chenyingfang entry gpio_led_probe \n");
	...
	for (i = 0; i < 3; i++) {	/* red, green, trickle*/
		if((!strcmp(STATUS_LED->blink_led[i].led.name, "trickle_red")) && (!gpio_is_valid(STATUS_LED->trickle_red_led)))
			continue;
