...
static unsigned char CTPM_FW_CW[] = {
	#include "FT_Upgrade_App_Chuangwei.i"
};
static unsigned char CTPM_FW_HLT[] = {
	#include "FT_Upgrade_App_Helitai.i"
};
...
* Return: None
*******************************************************************************/
void fts_update_fw_vendor_id(struct i2c_client *client/*fts_ts_data *data*/)
{
	...
	err = fts_i2c_read(client, &reg_addr, 1, &fw_vendor_id/*&data->fw_vendor_id*/, 1);
	if (err < 0)
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	{	   
		CTPM_FW = CTPM_FW_CW;	    
		fw_filenth = sizeof(CTPM_FW_CW);	
	}	
	else if(fw_vendor_id == FTS_VENDOR_ID_HELITAI)	
	{	    
		CTPM_FW = CTPM_FW_HLT;	    
		fw_filenth = sizeof(CTPM_FW_HLT);	
	}	
	//fw_filenth = sizeof(CTPM_FW);	
	//end
...
int fts_ctpm_get_i_file_ver(void)
{
	...
	{	    
	    CTPM_FW = CTPM_FW_CW;	   
	    ui_sz = sizeof(CTPM_FW_CW);	
	}	
	else if(fw_vendor_id == FTS_VENDOR_ID_HELITAI)	
	{	    
		CTPM_FW = CTPM_FW_HLT; 
		ui_sz = sizeof(CTPM_FW_HLT);	
	}   
	else	
	{	    
		return 0x00;	
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, fw_len/*sizeof(CTPM_FW)*/);
		if (i_ret != 0)
		{
