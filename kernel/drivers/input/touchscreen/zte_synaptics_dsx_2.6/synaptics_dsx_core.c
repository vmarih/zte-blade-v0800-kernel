...
bool tp_enable_wakeup_gesture_state = false;
//zz
static struct kobject *synaptics_tp_wake_gesture_kobj;
static struct platform_device *pdev_syn = NULL;
//zz
...
static struct attribute *synaptics_wake_gesture_attributes[] = {	
	&dev_attr_wake_gesture.attr,		
	NULL
};
static struct attribute_group synaptics_wake_gesture_attribute_group = {	
	.attrs = synaptics_wake_gesture_attributes
};
...
static ssize_t synaptics_rmi4_wake_gesture_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(pdev_syn);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
	    dev_err(rmi4_data->pdev->dev.parent,
				"%s: can not read input\n",
				__func__);
		return -EINVAL;
	}

	input = input == 49 ? 1 : 0;
    //printk("syna f11 = %d,f12 = %d\n",rmi4_data->f11_wakeup_gesture,rmi4_data->f12_wakeup_gesture);
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
	{
		rmi4_data->enable_wakeup_gesture = input;
		tp_enable_wakeup_gesture_state = input;
	}
	return count;
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
    ...
    //printk("syna,suspend = %d,enable_wakeup_gesture = %d\n",rmi4_data->suspend,rmi4_data->enable_wakeup_gesture);
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
...
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
		snprintf(buf, PAGE_SIZE, "dsx_gpio_%d\n", gpio);
...
//zz add by wkh for touch info

static struct proc_dir_entry *synaptics_info_proc_entry = NULL;

static const struct file_operations synaptics_info_fops = {
	.owner = THIS_MODULE,
	.read = synaptics_info_read_proc,
	.write = synaptics_info_write_proc,
};

static s32 synaptics_init_info_node(void)
{
	synaptics_info_proc_entry = proc_create("touchscreen_info", 0666, NULL, &synaptics_info_fops);
	if (synaptics_info_proc_entry == NULL) {
		pr_err("%s: Create proc entry /proc/touchscreen_info FAILED!\n",__func__);
		return -1;
	}
	pr_err("%s: Create proc entry /proc/touchscreen_info SUCCESS!\n",__func__);
	return 0;
}

ssize_t synaptics_info_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
        char szOut[100] = {0};
	u32 nLength = 0;
	...
	if (*ppos) {
        ...
        (void)getConfigId(szOut);
	
        pr_err("%s: szOut is  %s\n",__func__, szOut);
	nLength = snprintf(page, 100,"%s\n", szOut);
	*ppos += nLength;
	return nLength;
}

ssize_t synaptics_info_write_proc(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
    pr_err("%s: write count %ld\n",__func__, (unsigned long)count);
    return count;
}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	// add by wkh for create tp_wake_gesture
		synaptics_tp_wake_gesture_kobj = kobject_create_and_add("tp_wake_gesture", NULL);	 
		if (!synaptics_tp_wake_gesture_kobj)	
		{		 
			retval = -EINVAL;		 
			printk("syna--ERROR Unable to create shtsc_wake_gesture_kobj\n");	 
		}	 
		retval = sysfs_create_group(synaptics_tp_wake_gesture_kobj, &synaptics_wake_gesture_attribute_group);	 
		if (0 != retval)	 
		{		 
			printk("syna--ERROR: sysfs_create_group() failed.\n");		  
			//sysfs_remove_group(&client->dev.kobj, &gt9xx_wake_gesture_attribute_group);		 
			kobject_put(synaptics_tp_wake_gesture_kobj);		
			//return -EIO;	
		}	  
		else	 
		{		 
...
if(synaptics_tp_wake_gesture_kobj)
	{
		sysfs_remove_group(synaptics_tp_wake_gesture_kobj, &synaptics_wake_gesture_attribute_group);	
		kobject_put(synaptics_tp_wake_gesture_kobj);
	}
...
exit:
	...
	//printk("syna,synaptics_rmi4_suspend,early enter here\n");
...
exit:
	...
	rmi4_data->suspend = false;
	//printk("syna,synaptics_rmi4_resume early,enter here\n");
...
exit:
	...
	rmi4_data->suspend = true;
	//printk("syna,synaptics_rmi4_suspend,enter here\n");
...
exit:
	...
	rmi4_data->suspend = false;
	//printk("syna,synaptics_rmi4_resume,enter here\n");
