...
int getConfigId(char *buf)
{
	...
	/* device config id */
	retval = synaptics_rmi4_reg_read(rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,config_id_size);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to read device config ID\n",__func__);
		return retval;
	}

        dev_err(rmi4_data->pdev->dev.parent,"%s: config_id[0] is %d     config_id[1] is %d  \n",__func__,config_id[0],config_id[1] );
	dev_err(rmi4_data->pdev->dev.parent,"%s: config_id[2] is %d     config_id[3] is %d  \n",__func__,config_id[2],config_id[3] );
	
        if(config_id[0] == 88 && config_id[1] == 71)
        {
	     return snprintf(buf, 100, "TD4310+TMA(%d.%d.%d.%d)\n",config_id[0], config_id[1], config_id[2], config_id[3]);
        }
        else if(config_id[0] == 87 && config_id[1] == 70)
        {
	    return snprintf(buf, 100, "TD4300+BOE(%d.%d.%d.%d)\n",config_id[0], config_id[1], config_id[2], config_id[3]);
        }
	else if(config_id[0] == 176 && config_id[1] == 0 && config_id[2] == 0 && config_id[3] == 1)
        {
            return snprintf(buf, 100, "TD4300+BOE(%d.%d.%d.%d)\n",config_id[0], config_id[1], config_id[2], config_id[3]);
        }

	 pr_err("%s: config id not match ! \n",__func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	u32 nLength = 0;
        ...
        nLength = snprintf(buf,100, "%s\n", szOut);
        pr_err("%s: nLength = %d ,szOut is  %s  \n",__func__,nLength, szOut);
        return nLength;			
}
//end
