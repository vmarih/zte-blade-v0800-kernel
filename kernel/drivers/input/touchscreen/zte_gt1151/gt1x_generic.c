...
#define GOODIX_SYSFS_DIR      "tp_glove_mode"
...
static ssize_t gt1x_info_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gt1x_info_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *gt1x_debug_proc_entry = NULL;
static struct proc_dir_entry *gt1x_info_proc_entry = NULL;
...
static const struct file_operations gt1x_info_fops = {
	.owner = THIS_MODULE,
	.read = gt1x_info_read_proc,
	.write = gt1x_info_write_proc,
};


//add by wkh begin
static s32 gt1x_init_info_node(void)
{
	gt1x_info_proc_entry = proc_create(GT1X_INFO_PROC_FILE, 0666, NULL, &gt1x_info_fops);
	if (gt1x_info_proc_entry == NULL) {
		GTP_ERROR("Create proc entry /proc/%s FAILED!", GT1X_INFO_PROC_FILE);
		return -1;
	}
	GTP_INFO("Created proc entry /proc/%s.", GT1X_INFO_PROC_FILE);
	return 0;
}
...
static void gt1x_deinit_info_node(void)
{
	if (gt1x_info_proc_entry != NULL) {
		remove_proc_entry(GT1X_INFO_PROC_FILE, NULL);
	}
}

//add by wkh begin
static ssize_t gt1x_info_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	char *ptr = page;
	...
	/* Touch PID & VID */
	ptr += sprintf(ptr, "==== GT1151 Version Info ====\n");

	ptr += sprintf(ptr, "firmware version: %06X\n", gt1x_version.patch_id);

	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, &buf[0], 1);
        if(ret)
	GTP_INFO("GTP sensor_id = %d\n",gt1x_version.sensor_id);
    //module info
	switch (gt1x_version.sensor_id){
	case 0x0:
		ptr += sprintf(ptr,  "module,yili(0x%x)\n", buf[0]);//yili
		break;
	case 0x2:	
		ptr += sprintf(ptr,  "module,junda(0x%x)\n", buf[0]);//junda
		break;
	default:
		ptr += sprintf(ptr,  "module,unknown(0x%x)\n", buf[0]);
		break;
	}
#if 0
	gt1x_i2c_read(GTP_REG_FW_CHK_MAINSYS, temp_data, 12);
...
static ssize_t gt1x_info_write_proc(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
...
static struct kobj_attribute sc_attr = 
    __ATTR(state, S_IWUSR/*S_IWUGO*/ | S_IRUGO, smart_cover_show, smart_cover_store);
static int gt1x_smart_cover_init(void)
{
    ...
    #endif
    if (!gt1x_sc_dev->kobj)
        gt1x_sc_dev->kobj = kobject_create_and_add(/*"smartcover"*/GOODIX_SYSFS_DIR, NULL/*sysfs_rootdir*/);
        if (!gt1x_sc_dev->kobj) {
            err = -3;
            goto exit_free_mem;
    }
