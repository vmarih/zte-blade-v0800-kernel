...
static struct kobject *gt1x_tp_wake_gesture_kobj;
#if 0
...
#endif
static ssize_t gt1x_wake_gesture_show(struct device *dev,
...
static ssize_t gt1x_wake_gesture_store(struct device *dev,
...
static DEVICE_ATTR(wake_gesture, (S_IRUGO | S_IWUSR | S_IWGRP),  gt1x_wake_gesture_show,  gt1x_wake_gesture_store); 

static struct attribute *gt1x_wake_gesture_attributes[] = {	
	&dev_attr_wake_gesture.attr,		
	NULL
};
static struct attribute_group gt1x_wake_gesture_attribute_group = {	
	.attrs = gt1x_wake_gesture_attributes
};
...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    switch (doze_buf[0]){
	case 0xCC:
	    GTP_DEBUG("Gesture:doubleclick\n");
		input_report_key(dev, KEY_GESTURE_DOUBLEC, 1);
		input_sync(dev);
		input_report_key(dev, KEY_GESTURE_DOUBLEC, 0);
	input_sync(dev);
		break;
	case 0xBB:
	GTP_DEBUG("Gesture:slide left\n");
		input_report_key(dev, KEY_GESTURE_LEFT, 1);
		input_sync(dev);
		input_report_key(dev, KEY_GESTURE_LEFT, 0);
		input_sync(dev);
		break;
	case 0xAA:
	GTP_DEBUG("Gesture:slide right\n");
		input_report_key(dev, KEY_GESTURE_RIGHT, 1);
	input_sync(dev);
		input_report_key(dev, KEY_GESTURE_RIGHT, 0);
	input_sync(dev);
		break;
    }
...
s32 gt1x_init_node(void)
{
    ...
    memset(gestures_flag, 0xff, sizeof(gestures_flag));
    memset((u8 *) & gesture_data, 0, sizeof(st_gesture_data));  
    gt1x_tp_wake_gesture_kobj = kobject_create_and_add("tp_wake_gesture", NULL);    
	if (!gt1x_tp_wake_gesture_kobj)    
	{        
	    err = -EINVAL;        
		GTP_ERROR("GTP--ERROR Unable to create shtsc_wake_gesture_kobj\n");    
	}    
	err = sysfs_create_group(gt1x_tp_wake_gesture_kobj, &gt1x_wake_gesture_attribute_group);    
	if (0 != err)     
	{        
	    GTP_ERROR("GTP--ERROR: sysfs_create_group() failed.\n");        
		//sysfs_remove_group(&client->dev.kobj, &gt9xx_wake_gesture_attribute_group);        
		kobject_put(gt1x_tp_wake_gesture_kobj);        
		return -EIO;    
	}     
	else     
	{        
		GTP_ERROR("GTP:sysfs_create_group() succeeded.\n");    
	}    
	return err;
#endif
...
void gt1x_deinit_node(void)
{
	...
	if(gt1x_tp_wake_gesture_kobj)
	{
		sysfs_remove_group(gt1x_tp_wake_gesture_kobj, &gt1x_wake_gesture_attribute_group);	
		kobject_put(gt1x_tp_wake_gesture_kobj);
	}
#endif
