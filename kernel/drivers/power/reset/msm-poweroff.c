...
{
	if(get_option(&str, &zfg_dump_switch))
	    {
	    printk("zfg_dump_switch = %d\n", zfg_dump_switch);
	    return 0;
	  }
	else
	    {
	    printk("zfg_dump_switch get failed \n");
	    return -EINVAL;
	  }
}
early_param("zfg_dump_switch", dump_switch_setup);
