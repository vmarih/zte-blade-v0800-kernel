...
#define SPECIAL_HS_DETECT_TIME_MS 300 ///(2 * 1000)  modify for selfstick dll 20160616
				...
				enum snd_jack_types jack_type)
{
	...
	pr_err("%s: enter insertion %d hph_status %x\n",
					 ...
					 enum wcd_mbhc_plug_type plug_type)
{
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADPHONE) {
		...
		//wcd_mbhc_report_plug(mbhc, 1, SND_JACK_UNSUPPORTED);  //dll modify by qualcomm patch for selfstick
		if (mbhc->impedance_detect) {
				mbhc->mbhc_cb->compute_impedance(mbhc,
						&mbhc->zl, &mbhc->zr);
				pr_err("%s: dll test ----zl:%d,zr:%d \n", __func__,mbhc->zl, mbhc->zr);
				if ((mbhc->zl > 20000) && (mbhc->zr > 20000)) {
					pr_debug("%s: dll test ----special accessory \n", __func__);
					// Toggle switch back
					if (mbhc->mbhc_cfg->swap_gnd_mic &&
						mbhc->mbhc_cfg->swap_gnd_mic(codec)) {
						pr_debug("%s: dll+++++US_EU gpio present,flip switch again\n"
								, __func__);
					}
					...
					pr_err("%s: dll++++ headset is SND_JACK_UNSUPPORTED ;100", __func__);
			...
			enum wcd_mbhc_plug_type plug_type)
{
	...
	if (det_extn_cable_en && mbhc->is_extn_cable &&
		...
		if (plug_type == MBHC_PLUG_TYPE_HEADSET) {
			...
			pr_err("dll modify for selfstick cs mode\n");
			wcd_enable_curr_micbias(mbhc, WCD_MBHC_EN_CS);
		} else {
...
correct_plug_type:
	...
	if (plug_type == MBHC_PLUG_TYPE_HIGH_HPH &&
		...
		if (wcd_is_special_headset(mbhc)) {
			...
			pr_err("%s: liyang for selfie stick found %d\n",__func__, plug_type);
			plug_type = MBHC_PLUG_TYPE_HEADSET;
			goto report;
		}
